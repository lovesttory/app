...
if($this->isPlain()) {
	...
	$boundary = '--BOUNDARY_LIMIT_TT--';
	If (strpos($text, $boundary)){
	$stext = substr($text, 0, strpos($text, $boundary));
	$shtml = str_replace($boundary, '', substr($text, strpos($text, $boundary)));

	$mail->setBodyText($stext);
	$mail->setBodyHTML($shtml); 
} else {
	$mail->setBodyHTML($text);
	$mail->setBodyText('Questa email e\' stata inviata in modalita\' HTML. Visualizzare la parte HTML oppure usare un lettore compatibile');
}
}
// fine modifiche TT
