...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object
{
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
		...
		// sposta la data dell'ordine a fianco al numero ordine 
		// inizio modifiche TT
		$page->drawText(Mage::helper('sales')->__('Order Date: ') . Mage::helper('core')->formatDate($order->getCreatedAtStoreDate(), 'medium', false), 170, 770, 'UTF-8');
		// aggiungi all'indirizzo di fatturazione la partita iva o il codice fiscale
		$page->drawText(Mage::helper('sales')->__('P.I. / Cod. Fisc.: ') . $order->getData('customer_taxvat'), 350, 780, 'UTF-8');
		// fine momdifiche TT
       ...
       // sposta l'indirizzo di spedizione
	   // inizio modifiche TT
	   if (!$order->getIsVirtual()) {
			$y = 730 - (max(count($billingAddress), count($shippingAddress)) * 10 + 15);
	   } else {
			$y = 730 - (count($billingAddress) * 10 + 15 );
		}
		// fine momdifiche TT
        ...
        if (!$order->getIsVirtual()) {
            ...
            $page->setLineWidth(0.5);
// inizio modifiche TT
	$page->drawRectangle(25, $this->y-10, 275, $this->y-25);
	$page->drawRectangle(275, $this->y-10, 570, $this->y-25);

	$this->y -=15;
	$this->_setFontBold($page);
	$page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
	$page->drawText(Mage::helper('sales')->__('Payment Method'), 35, $this->y-5, 'UTF-8');
	$page->drawText(Mage::helper('sales')->__('Shipping Method:'), 285, $this->y-5 , 'UTF-8');
// fine momdifiche TT
        ...
        if (!$order->getIsVirtual()) {
            ...
            if (count($tracks)) {
                ...
                foreach ($tracks as $track) {
                    ...
                    // inizio modifiche TT
					if (preg_match('/custom?([0-1])/', $CarrierCode))
					// fine modifiche TT
                    {
    ...
    {
        $font = Zend_Pdf_Font::fontWithName('Helvetica-Bold');
        $object->setFont($font, $size);
        return $font;
    }
    ...
    {
        $font = Zend_Pdf_Font::fontWithName('Helvetica-BoldOblique');
        $object->setFont($font, $size);
        return $font;
    }
