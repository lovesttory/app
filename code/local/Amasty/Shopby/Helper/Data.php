...
class Amasty_Shopby_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    // modifica per scelta dimensione immagine
    public function init($productCollection, $imagetype='list')
    {
        ...
        foreach ($optionCollection as $opt){
            ...
            // modifica per scelta dimensione immagine
            if ($imagetype=='view') {$img  = $opt->getImgBig();} else {$img  = $opt->getImgMedium();}
     ...
     * @return unknown
     */
    /* modificato per permettere la scelta dell'immagine da mostrare, se il metodo viene chiamato con list
	 * allora mostra la piccola, se con view allora la grande 
	 */
    public function showLinks($product, $mode='view', $names=array(), $exclude=false)
    {
        if ('view' == $mode || 'list' == $mode){
            $this->init(array($product),$mode);    
        }
        $filters = $this->_getFilters();
